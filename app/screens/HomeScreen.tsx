        ...
        >
          ...
          >
            ...
            {messages.map((msg, idx) => {
              if (msg.type === 'system') {
                return (
                  <View key={idx} style={[styles.systemMsg, { marginBottom: 16 }]}><Text style={styles.systemMsgText}>{msg.text}</Text></View>
                );
              }
              if (msg.type === 'booking_detail') {
                return <View key={idx} style={{ marginBottom: 16 }}><DetailBooking {...msg.data} onEdit={() => setShowBooking(true)} onConfirm={() => router.push('/screens/ChatwithDriver')} /></View>;
              }
              if (msg.type === 'user') {
                return (
                  <View key={idx} style={{ flexDirection: 'row', justifyContent: 'flex-end', marginBottom: 16 }}>
                    <View style={{ backgroundColor: '#009CA6', borderRadius: 12, paddingVertical: 8, paddingHorizontal: 16, alignSelf: 'flex-end', maxWidth: '80%' }}>
              ...
              if (msg.type === 'car_status') {
                return (
                  <View key={idx} style={{ marginBottom: 16, borderRadius: 16, backgroundColor: '#fff', overflow: 'hidden', shadowColor: '#009CA6', shadowOpacity: 0.08, shadowRadius: 12, elevation: 3 }}>
                    {/* Bản đồ tĩnh */}
                    <View style={{ height: 160, backgroundColor: '#e0f7fa', borderTopLeftRadius: 16, borderTopRightRadius: 16, overflow: 'hidden', position: 'relative' }}>
                      <View style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, backgroundColor: '#e0f7fa', justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: '#009CA6', fontWeight: 'bold' }}>[Bản đồ tĩnh]</Text>
                      </View>
                      <View style={{ position: 'absolute', left: '50%', top: '60%', marginLeft: -24 }}>
                        <Ionicons name="car" size={48} color="#F4C95D" />
                      </View>
                    </View>
                    {/* Thông tin tài xế */}
                    <View style={{ flexDirection: 'row', alignItems: 'center', padding: 12, backgroundColor: '#fff', borderRadius: 12, margin: 12, marginBottom: 0, shadowColor: '#009CA6', shadowOpacity: 0.06, shadowRadius: 6, elevation: 1 }}>
                      <View style={{ width: 48, height: 48, borderRadius: 24, overflow: 'hidden', marginRight: 12, backgroundColor: '#eee' }}>
                        <Ionicons name="person" size={40} color="#009CA6" style={{ alignSelf: 'center', marginTop: 4 }} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontWeight: 'bold', color: '#222', fontSize: 15 }}>{msg.driver?.name}</Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
                          <Ionicons name="star" size={14} color="#F4C95D" />
                          <Text style={{ color: '#009CA6', fontSize: 13, marginLeft: 4 }}>{msg.driver?.rating}</Text>
                          <Text style={{ color: '#b0b0b0', fontSize: 13, marginLeft: 6 }}>({msg.driver?.reviews} đánh giá)</Text>
                        </View>
                        <Text style={{ color: '#009CA6', fontSize: 13, marginTop: 2 }}>Còn {msg.driver?.distance} mét.</Text>
                      </View>
                      <TouchableOpacity style={{ backgroundColor: '#e0f7fa', borderRadius: 20, padding: 8, marginLeft: 8 }}>
                    ...
                    <View style={{ padding: 12, paddingTop: 8 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                        ...
                        <View style={{ flex: 1, height: 6, backgroundColor: '#009CA6', borderRadius: 3 }} />
                      </View>
                      <View style={{ backgroundColor: '#f2f2f2', borderRadius: 8, padding: 10, marginBottom: 10 }}>
                        <Text style={{ color: '#F44336', fontWeight: 'bold', fontSize: 15 }}>Xe của bạn đang trên đường đến!</Text>
                        <Text style={{ color: '#222', fontSize: 14, marginTop: 2 }}>⏱️ Thời gian đến dự kiến: {msg.eta} phút.{"\n"}Trong lúc chờ đợi, bạn có thể:</Text>
                      </View>
                      <View style={{ flexDirection: 'row', gap: 10 }}>
                        <TouchableOpacity style={{ backgroundColor: '#009CA6', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 18, marginRight: 8 }}>
                          <Text style={{ color: '#fff', fontWeight: 'bold' }}>Thời tiết hôm nay</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={{ backgroundColor: '#F4C95D', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 18 }}>
                          <Text style={{ color: '#009CA6', fontWeight: 'bold' }}>Liên hệ tài xế</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                  </View>
